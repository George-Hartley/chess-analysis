...
function smoothEvals(evalArray) {
  return evalArray.map((val, i, arr) => {
    const prev = arr[i - 1] ?? val
    const next = arr[i + 1] ?? val
    return Math.round((prev + val + next) / 3)
  })
}
...
function buildLineChart(evalArray) {
  ...
  const displayArray = smoothEvals(storedEvalArray)
  ...
  const stepX = displayArray.length > 1
    ? w / (storedEvalArray.length - 1)
    : w

  const points = displayArray.map((evalCp, i) => ({
    x: i * stepX,
    y: evalToY(evalCp, h)
  }))
...
function highlightLinePoint(currentMoveIndex) {
  ...
  const w = lineChartCanvas.width
  const h = lineChartCanvas.height
  const smoothed = smoothEvals(storedEvalArray)
  ...
  if (currentMoveIndex >= 0 && currentMoveIndex < smoothed.length) {
